...
| Skill | 它到底幹嘛 |
|-------|----------|
| [gpt-image-gen](gpt-image-gen/) | 把 Codex 哄去幫你生圖。先擬一份精準的雙語 prompt，跟你來回改到你明確說「生」，才真的呼叫 Codex CLI 文生圖 — 那道硬拍板閘門是因為「看起來不錯」不等於「生」，而每張圖都燒真金白銀的額度。文生圖、圖生圖（丟張本機參考圖就能 codex `-i` 鎖臉/鎖角色），還有改圖 —— 去背或局部修一處，prompt 寫成「其他每一像素都不准動、只改 X」，交給你之前先驗尺寸、透明度與主體像素保真，因為模型拿到編輯任務會很樂意重新生成一張很像的給你。生圖給 jpg、改圖給 png（jpg 沒有 alpha 通道，而編輯結果不可重現），而且絕不主動把預覽視窗糊你臉上 |
