...
| [project-docs](project-docs/) | 把既有專案補成下一位接得下去的技術文件：掃描全專案、對照證據與缺口，更新有交叉連結的 Markdown、Mermaid 圖、介面／資料／維運與發布交接。沿用公司模板，未知就明說，增量更新；技術文件預設正體中文，GitHub 文件加英文摘要 |

### 生成與創作

| Skill | 它到底幹嘛 |
|-------|----------|
| [gpt-image-gen](gpt-image-gen/) | 先把模糊需求收斂成精準的雙語 prompt，跟你來回改到明確說「生」才真的執行；拍板後依宿主走原生路徑：Codex 直接呼叫目前 session 的 built-in `image_gen`，Claude Code 則維持背景 Codex CLI 流程，不再讓 Codex 疊一層 Codex。支援文生圖、圖生圖與精確改圖；本機 artifact 可用時會驗尺寸、透明度與像素保真，改圖一律保留 png。 |
...
| [prep-repo](prep-repo/) | 按實際交付物檢查去敏、安裝、文件與圖表渲染、相關測試和 CI；分清阻擋發布與一般修飾，讓審查過的檔案一路對到 commit、push、發布，不會為了一行普通署名去改寫歷史；驗收既定語系與英文摘要＋正體中文技術正文 |
