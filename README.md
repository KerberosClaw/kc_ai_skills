...
| [loop-engineer](loop-engineer/) | For when you want an agent to grind on something overnight without you hovering over it. Interview-style, it pins down a goal-driven evaluator-optimizer loop (generate -> grade itself -> iterate by reason-code), then emits a self-contained dispatch doc a fresh session runs blind while you just watch the green/yellow/red pings roll in. NOT a cron timer and NOT a recurring-push registrar -- it is the "go figure it out, only ping me if you are actually stuck" button. Channel-agnostic notifications (Telegram/Discord/Slack/iMessage), and it bakes in a "want an adversarial review before we ship?" gate -- because we got tired of remembering to ask ourselves |
